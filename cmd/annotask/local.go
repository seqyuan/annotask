...
func runLocalMode(config *Config, args []string) {
	...
	mem := 1
	h_vmem := 1
	// Local mode doesn't use DRMAA, so mem/h_vmem/queue/sge-project/pesmp flags are not relevant
	runTasks(config, *opt_i, *opt_l, *opt_p, *opt_project, ModeLocal, config.Defaults.CPU, mem, h_vmem, false, false, "", "", false)
}

// runTasks is the common function to run tasks in both modes
func runTasks(config *Config, infile string, line, thread int, project string, mode JobMode, cpu, mem, h_vmem int, userSetMem, userSetHvmem bool, queue string, sgeProject string, usePesmp bool) {
	...
	for retryCount := 0; retryCount < maxRetries; retryCount++ {
		IlterCommand(ctx, dbObj, thread, need2run, mode, cpu, mem, h_vmem, userSetMem, userSetHvmem, queue, sgeProject, usePesmp, write_pool)
		need2run = GetNeed2Run(dbObj)
		if len(need2run) == 0 {
			break
		}
